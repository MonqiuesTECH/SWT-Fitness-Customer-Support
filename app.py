...
# - Human handoff via Calendly API slot buttons (or public link fallback)
# - Lead capture to Google Sheets (asks name/email/phone on arrival; uses leads.py)

from __future__ import annotations

import io, os, json, re, time, textwrap, datetime as dt, tempfile
...
# Local modules you already have in the repo
...
CALENDLY_EVENT_TYPE = st.secrets.get("CALENDLY_EVENT_TYPE", "").strip()  # API URI (preferred: renders slot buttons)
...
# ---------- Storage (use a writable temp dir on Streamlit Cloud) ----------
# You can override via Secrets with DATA_DIR="/app/data" if desired.
...
ss.setdefault("chat_history", [])        # list[(role, text)]
...
def show_manager_slots():
    ...
    # Preferred: event-type API URI -> live slot buttons
    ...
    # Fallback: public booking page link
    if CALENDLY_URL:
        st.write(f'<a href="{CALENDLY_URL}" target="_blank" rel="noopener">Open booking page</a>',
                 unsafe_allow_html=True)
    else:
